            ...
            {({ isActive }) => (
              <motion.div
                className={cn(
                  ...
                  isActive
                    ? 'bg-indigo-600/20 text-indigo-500 border border-indigo-600/30'
        ...
        <NavLink to="/settings">
          {({ isActive }) => (
            <motion.div
              ...
              style={!isActive ? { color: 'var(--tf-text-muted)' } : { background: 'var(--tf-bg-tertiary)' }}
              onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'var(--tf-bg-tertiary)' }}
              onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = '' }}
              whileHover={{ x: 1 }}
              whileTap={{ scale: 0.98 }}
